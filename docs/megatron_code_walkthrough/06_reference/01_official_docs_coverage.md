...
| Data preparation / Data loading | `01_foundations/00_ecosystem_install_and_data.md`、训练主线 | 详解 |
...
| Transformer / GPT/BERT/T5 API | 模型与 ModuleSpec、模型谱系 | 详解 |
...
| Megatron MoE / router replay / paged stash | MoE/MLA/MTP、深读 | 详解 |
| MTP / MLA | MoE/MLA/MTP | 归纳+源码入口 |
| Megatron-FSDP | `03_state_and_memory/02_megatron_fsdp.md` | 归纳+生命周期 |
| CPU/fine-grained activation offload | 显存、确定性与 CUDA Graph；深读 | 详解 |
| CUDA Graph | 显存、确定性与 CUDA Graph | 归纳 |
| Deterministic training | 显存、确定性与 CUDA Graph | 详解 |
