...
> 参考层：保留原笔记的架构演进资料与出处。具体型号、发布日期、版本号和性能结论属于原资料记录，本轮只做内容归位，未逐项重新核验；不作为当前 H20 环境的规格承诺。基础机制见物理基础与系统专题，型号资料按需要回查。
...
<a id="概念辨析迁移自原笔记-02"></a>

## 概念辨析

<a id="02从-a100--h100--b200--rubinnvidia-gpu-架构为什么不断这样变化"></a>

### 从 A100 → H100 → B200 → Rubin：NVIDIA GPU 架构为什么不断这样变化？
...
##### WGMMA：矩阵计算的协作粒度从 Warp 扩大到 Warp Group
...
##### Thread Block Cluster：Block 不再完全是“孤岛”
...
##### HBM3e → HBM4：decode 已经明确成为 memory subsystem 问题
...
##### enhanced TMA：MoE 让“tensor descriptor 管理”本身都成为成本
...
##### activation sparsity + adaptive compression：不再只稀疏权重，也开始稀疏中间激活
...
##### 更细粒度 dependent-kernel triggering：Kernel boundary 本身开始成为瓶颈
...
##### NVLink 6：GPU-GPU communication 越来越像“GPU 内部数据通路”的延伸
