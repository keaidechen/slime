...
> 前置：会运行 Python。目标：把一次实验的机器、进程、软件和输入记录完整。

<a id="1-从你输入的命令追到-gpu"></a>
...
<a id="21-nvidia-smi查看-gpu-和驱动"></a>

### 1 `nvidia-smi`：查看 GPU 和驱动
...
<a id="33-netstat-和-ss查看端口与网络连接"></a>

### 3 `netstat` 和 `ss`：查看端口与网络连接
...
相关条目：[PyTorch 张量与自动微分](../01_Math_Numerics/PyTorch张量与自动微分.md)。具体采集软件安装见[软件教程](../../docs/performance_analysis_guide/software_tutorials.md)，完整实验记录见[可信基线](../../docs/performance_analysis_guide/01_baseline.md)。
