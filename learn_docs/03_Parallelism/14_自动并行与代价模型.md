...
假设只比较两个合法方案 A/B。A 节省 4 GB 常驻显存但增加 8 ms 通信，B 多占显存但减少通信。若显存上限被触发，B 根本不在可行解中；若 A 的新增通信完全暴露，B 可能更快。规划先判断可行性，再判断目标值。

代价表至少包含参数/activation 峰值、计算时间、通信 bytes/次数、拓扑和可重叠依赖。把所有事件耗时相加会高估重叠后的总时间；只用峰值算力又会低估小 shape 和启动成本。

练习：用自己的两三个已测配置验证预测排序，而不只验证绝对数字。若排序错误，先找缺失的成本项，再扩大搜索空间。
