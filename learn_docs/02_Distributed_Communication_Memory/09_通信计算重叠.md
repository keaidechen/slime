...
> **首读/二读**：异步不等于重叠；用 Stream/Event 与 NCCL trace 验证。
...
假设第 2 层反向产生梯度需要 6 ms，第 1 层反向需要 5 ms，第 2 层梯度通信需要 4 ms。若通信能在第 2 层梯度 ready 后启动，并与第 1 层反向重叠，末尾暴露时间可能接近 0；若先等整个 backward 完成才通信，至少再付出这 4 ms。

这是理想示意，实际还有第 1 层梯度通信、启动延迟和资源竞争。两个 stream 只是表达潜在并发，通信若占用计算资源或带宽，也可能拖慢本来独立的 kernel。

练习：画 producer（写 buffer）、collective（读/写 buffer）、consumer 三个节点。标出 buffer 何时允许复用。比较总 step 时间与每事件累计时间，只有关键路径缩短才证明 overlap 有端到端收益。
