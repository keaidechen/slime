...
> 前置：GPU 执行模型、tensor 连续布局与正确计时。目标是理解并验证一个小算子，不以超越成熟库为完成条件。

<a id="1-先缩小要实现的数学问题"></a>
...
<a id="5-从这个例子进入-kernel-专题"></a>

## 从这个例子进入 Kernel 专题

| 观察 | 下一步问题 | 阅读 |
|---|---|---|
| 小输入时间几乎不变 | 固定 launch 成本占多大比例 | [CUDA 执行](../03_Hardware_Systems/CUDA执行模型与调度.md) |
| 大输入随 n 增长 | 逻辑带宽与真实带宽怎样比较 | [内存与搬运](GPU内存编程与数据搬运.md) |
| 改 BLOCK 后速度变化 | 资源、并行粒度与尾块浪费 | [Kernel 实验](实验与排查手册.md) |
| 两个 elementwise 串联 | 能否减少一次中间读写 | [PyTorch 编译栈](../06_Compilers/PyTorch编译栈.md) |
