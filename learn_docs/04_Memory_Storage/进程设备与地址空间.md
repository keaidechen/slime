...
`topo` 是拓扑子命令；`-m` 输出 GPU/NIC 连接矩阵，`-p2p r` 查 GPU pair 的 P2P read capability，`-p2p w` 查 P2P write capability。P2P 结果也是矩阵：行和列分别列出 GPU，交叉格是这一对设备的能力状态。它报告驱动识别到的能力，不是实测带宽；详细读法见[节点内 GPU 互联](../07_Communication/节点内GPU互联.md#拓扑输出如何读)。
...
<a id="概念辨析迁移自原笔记-08"></a>

## 概念辨析

<a id="08python-一个进程能不能管理多个-gpu为什么训练通常选择一进程一卡"></a>

### Python 一个进程能不能管理多个 GPU？为什么训练通常选择“一进程一卡”？
