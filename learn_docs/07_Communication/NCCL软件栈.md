...
<a id="228-以后的-device-side-路线"></a>

<a id="28-以后的-device-side-路线"></a>
...
应用说明参与者、buffer 和 collective；库选择可行算法、协议与传输路径；底层受 GPU 拓扑、NIC、内存注册与设备进度限制。一次性能异常应分层定位，不能把所有等待归因于网络带宽。

优化通信启动与传输路径后，buffer 生命周期、消费者可见性和并发资源竞争仍要验证。调环境变量前保存默认基线，并记录它针对的具体假设。版本前沿内容是原稿的版本快照，当前部署需要重新确认支持情况。

参见[DMA 与 RDMA 完成语义](DMA与RDMA完成语义.md)。
