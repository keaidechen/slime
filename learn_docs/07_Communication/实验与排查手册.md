...
| 命令 | 作用 | 重点看什么 |
|---|---|---|
| `nvidia-smi topo -m` | 显示 GPU/NIC 拓扑矩阵 | GPU–GPU 路径、GPU–NIC 距离、CPU/NUMA affinity；读法见[节点内 GPU 互联](节点内GPU互联.md#拓扑输出如何读) |
| `nvidia-smi -q` | query（查询）详细 GPU 状态 | GPU UUID/PCI bus ID、显存、温度、功耗、时钟、ECC 和重放/错误计数；输出很长，应和时间戳一起保存 |
| `ibstat` | 查询本机 InfiniBand device/port 的基本状态 | CA/HCA 名、port `State`、`Physical state`、`Rate`、链路宽度与 GUID/LID |
| `ibdev2netdev` | 把 RDMA device/port 映射到 Linux 网卡 | 例如 `mlx5_0 port 1 ==> ib0 (Up)`；这能避免把 HCA 口名和 `eth0`/`ib0` 等 netdev 名混在一起 |
| `rdma link` | 列出 RDMA link 属性，等价的显式写法是 `rdma link show` | link 名和 port index、`state`、`physical_state`、关联的 netdev |

CA/HCA 是 Channel Adapter/Host Channel Adapter，可先理解为服务器的 RDMA 网络适配器。`State: Active`/`Physical state: LinkUp` 只说明端口已经起来，不能证明路由、RDMA 数据面和 NCCL 都正常。命令不存在时，先确认镜像是否安装 `infiniband-diags`/`rdma-core` 等工具；在容器内还要确认相关设备和 sysfs 是否已映射进来。
...
```bash
NCCL_DEBUG=INFO ./build/all_reduce_perf -b 8 -e 8G -f 2 -g 2
```

这是一个“单节点、两张可见 GPU”的具体例子：

| 输入部分 | 含义 |
|---|---|
| `NCCL_DEBUG=INFO` | 只为这一次命令设置 NCCL 日志级别；日志会包含版本、网卡和拓扑/传输选择等线索 |
| `./build/all_reduce_perf` | 运行当前目录下 `build` 里的 nccl-tests AllReduce 可执行文件；`./` 表示“从当前目录开始找” |
| `-b 8` | 最小消息大小为 8 Byte |
| `-e 8G` | 最大消息大小为 8 GiB |
| `-f 2` | 每次把消息大小乘 2，即 8 B、16 B、32 B……直到 8 GiB |
| `-g 2` | 每个测试线程使用 2 张 GPU；本命令因而有 2 个 NCCL rank |

正常输出会对每个消息大小给出 `size`、`count`、`type`、`time`、`algbw`、`busbw` 和正确性错误数等列，并分别报告 out-of-place 与 in-place 形态。`time` 是平均操作时间，`algbw` 是按消息大小/时间计算的算法带宽，`busbw` 根据 collective 通信量做了校正，更适合与链路能力比较。正确性错误数应为 0；只看带宽、不看错误列，可能把错误结果当成性能结果。

`-e 8G` 会分配大于单个消息的总 buffer，显存不足时可先把上限改成 `-e 128M`。若只运行 `-g 1` 且没有 MPI/其他多进程启动器，总 rank 数只有 1，可用于检查程序能否启动，但不能作为 GPU 之间的通信带宽基线。
...
- [nccl-tests 性能列解释](https://github.com/NVIDIA/nccl-tests/blob/master/doc/PERFORMANCE.md)
- [`rdma link` 手册](https://man7.org/linux/man-pages/man8/rdma-link.8.html)
