...
## 入门例子：逐段看一次跨节点 tensor 传输

先画 A 节点 GPU HBM → A 节点 NIC → 网络 → B 节点 NIC → B 节点 GPU HBM。若某一段不支持直接设备访问，路径可能引入 host buffer。RDMA 描述远端内存访问机制，GPUDirect RDMA 关注 NIC 对 GPU 内存的路径；它们不是“跨节点一定零拷贝”的承诺。

| 观察 | 候选解释 | 下一份证据 |
|---|---|---|
| 同节点快，跨节点慢 | NIC/网络/亲和性或负载变化 | 相同消息的单节点与双节点对照 |
| 同节点不同 GPU 对表现不同 | PCIe/root/NUMA 或 NVLink 路径差异 | GPU–NIC 与 GPU–GPU 拓扑 |
| 只有一台机器慢 | 配置、争用、硬件或进程到达偏斜 | 同配置复测并对齐 rank 时间线 |

第一次实验只记录路径、消息大小和持续带宽，不盲调网络参数。验收：解释“数据最终到达”与“走了预期路径”的区别，并能指出当前还缺哪项证据。
...
- “ping 通说明 RoCE 正常”：常规 ICMP ping 不验证 RDMA verbs、PFC/ECN、GID、MTU 或 GPU Direct。
