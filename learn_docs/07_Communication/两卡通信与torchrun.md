...
> 前置：[进程与异步](../03_Hardware_Systems/进程线程与四种异步.md)、tensor 的 shape/dtype。CPU 可先运行 Gloo 演示；H20 上用 NCCL。

<a id="1-一个-launcher两个-python-进程"></a>

## 一个 launcher，两个 Python 进程
...
一个简化模型是 `T ≈ α + bytes / bandwidth`：小消息更容易被固定延迟支配，大消息更容易暴露带宽。真实 collective 还要算算法步骤数、链路复用与到达偏斜，见[通信性能模型](通信性能模型.md)。
...
下一步：[通信原语](通信原语与Collective.md) → [DP/DDP](../08_Parallelism/DP与DDP.md) → [TP](../08_Parallelism/TensorParallelism.md)。API 依据：[PyTorch distributed](https://docs.pytorch.org/docs/stable/distributed.html)、[torchrun](https://docs.pytorch.org/docs/stable/elastic/run.html)。
