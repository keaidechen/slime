...
> 前置：[四种异步](../03_Hardware_Systems/进程线程与四种异步.md)、进程与设备映射。先在 CPU 上观察调度，不需要加载大模型。

<a id="1-taskactor-与-objectref"></a>
...
[异步队列与权重发布推演](推演_异步队列与权重发布.md)：在途任务、完成队列与实际源码背压。
